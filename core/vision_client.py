...
def classify_input_with_vision(uploaded_files, api_key: str) -> dict:
    """จำแนกชนิดหลักฐานก่อนเลือก Engine โดยขอเฉพาะผลจำแนก ไม่ให้สร้างรายงาน"""
    parts = []
    input_names = []
    for file in uploaded_files:
        ...
        if filename.endswith(".txt"):
            try:
                notes = file.read().decode("utf-8", errors="ignore")
                file.seek(0)
                parts.append({"text": f"FIELD NOTES ({filename}):\n{notes[:4000]}"})
            except Exception:
                pass
        elif any(filename.endswith(ext) for ext in [".png", ".jpg", ".jpeg", ".webp"]):
            try:
                image_data = optimize_image(file.read(), max_dim=2400)
                file.seek(0)
                parts.append({"inline_data": {"mime_type": "image/png", "data": image_data}})
            except Exception:
                file.seek(0)

    parts.insert(0, {"text": f"""
จำแนกชุดหลักฐานนี้เพื่อเลือกประเภทรายงานเท่านั้น ห้ามวิเคราะห์ปัญหาและห้ามสร้างตัวเลข
ประเภทที่เลือกได้: VISUAL_SURVEY, THERMAL_SCAN, STRING_MEASUREMENT,
PREVENTIVE_MAINTENANCE, DIAGNOSTIC หรือ MIXED_INPUT

หลักเกณฑ์:
- ภาพ RGB มุมสูง/หลังคา/โรงงานที่ไม่มีข้อมูลวัด เป็น VISUAL_SURVEY
- ภาพสีความร้อนหรือ IR เป็น THERMAL_SCAN
- ภาพ Clamp Meter หรือค่ากระแส String เป็น STRING_MEASUREMENT
- Checklist งานตรวจตามรอบ เป็น PREVENTIVE_MAINTENANCE
- ภาพหน้าจอ Inverter/SmartLogger/Alarm เป็น DIAGNOSTIC
- ถ้าหลักฐานมีหลายประเภทชัดเจน ให้ตอบ MIXED_INPUT
- ถ้าไม่แน่ใจ ให้ตอบ confidence ต่ำ ห้ามเดา

ชื่อไฟล์: {", ".join(input_names)}
ตอบ JSON เท่านั้น:
{{
  "report_type": "ประเภทเดียวจากรายการด้านบน",
  "confidence": 0.0,
  "mixed": false,
  "evidence": ["สิ่งที่เห็นหรืออ่านข้อความได้จริงเท่านั้น"]
}}
"""})
    result = execute_gemini_vision(parts, api_key)
    allowed = {"VISUAL_SURVEY", "THERMAL_SCAN", "STRING_MEASUREMENT", "PREVENTIVE_MAINTENANCE", "DIAGNOSTIC", "MIXED_INPUT"}
    result_type = str(result.get("report_type", "")).strip().upper()
    if result_type not in allowed:
        raise ValueError("Vision Classifier ส่งประเภทงานที่ไม่รองรับ")
    result["report_type"] = result_type
    result["confidence"] = max(0.0, min(1.0, float(result.get("confidence", 0.0))))
    result["mixed"] = bool(result.get("mixed", result_type == "MIXED_INPUT"))
    return result